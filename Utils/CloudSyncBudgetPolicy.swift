...
enum CloudSyncBudgetPolicy {
    /// Matches existing per-key cap used by `CloudSyncStore` on both platforms.
    static let maxPerKeyBytes = 512 * 1024
    /// Apple documents ~1 MB total KVS budget; reserve headroom for system metadata keys.
    ...
    static func footprints(from store: NSUbiquitousKeyValueStore) -> [KeyFootprint] {
        ...
        var dataKeys = Set(keys.filter { !$0.hasSuffix(".__modifiedAt") })
