...
**Canonical numbered filename:** `02-MASTER_IOS_FULL_DEEP_COMPREHENSIVE_AUDIT_COMMAND_V1.7.md`

---

# MASTER CURSOR / CODEX COMMAND — DIR DIVING iOS FULL DEEP COMPREHENSIVE AUDIT — V1.7
...
```text
Docs/MASTER_IOS_APNEA_FULL_DEEP_AUDIT_CURRENT.md
Docs/MASTER_IOS_APNEA_FEATURE_INVENTORY_CURRENT.csv
Docs/MASTER_IOS_APNEA_SETTINGS_OWNERSHIP_MATRIX_CURRENT.csv
Docs/MASTER_IOS_APNEA_LOGBOOK_OWNERSHIP_MATRIX_CURRENT.csv
Docs/MASTER_IOS_APNEA_SYNC_SCHEMA_MATRIX_CURRENT.csv
Docs/MASTER_IOS_ALGORITHMIC_PARITY_WITH_WATCH_GATE_CURRENT.md
```
...
# V1.7 IOS SPECIFIC SCOPE — SNORKELING REMEDIATION, CCR ACK AND EQUIPMENT GAS UI

Audit latest iOS work:

```text
Snorkeling pending queue persistence and namespace safety
Snorkeling session sync status on list/detail/planner
Snorkeling source labels Watch / Manual / Imported
settings re-send required banner
planned-vs-actual non-safety wording
CCR independent acknowledgement and mode-aware policy
equipment gas/cylinder dedicated section and creation path
demo/fake log exclusion from real/unified logbooks
```

Additional outputs:

```text
Docs/MASTER_IOS_V1_7_SNORKELING_REMEDIATION_AUDIT_CURRENT.md
Docs/MASTER_IOS_V1_7_CCR_ACKNOWLEDGEMENT_AUDIT_CURRENT.md
Docs/MASTER_IOS_V1_7_EQUIPMENT_GAS_UI_AUDIT_CURRENT.md
Docs/MASTER_IOS_V1_7_DEMO_LOGBOOK_FIX_AUDIT_CURRENT.md
```
...
# 6B. POST-REMEDIATION iOS / WATCH GF AND SYNC FOCUS

This audit must additionally verify whether remediation has closed these consolidated software findings where they affect iOS:

```text
CONS-002 — iOS GF preset parity with Watch
CONS-003 — iOS inFlightOutboundSessionIDs cleanup after failed ACK
CONS-004 — symmetric Watch↔iOS diveImportAck
CONS-005 — unsigned tombstone hardening
CONS-028 / CONS-040 — activity architecture, Settings and Logbook isolation if present
```

Additional required outputs:

```text
Docs/MASTER_IOS_POST_REMEDIATION_GF_SYNC_VERIFICATION_CURRENT.md
Docs/MASTER_IOS_GF_PRESET_PARITY_POST_REMEDIATION_MATRIX_CURRENT.csv
Docs/MASTER_IOS_SYNC_ACK_POST_REMEDIATION_MATRIX_CURRENT.csv
Docs/MASTER_IOS_TOMBSTONE_SECURITY_POST_REMEDIATION_MATRIX_CURRENT.csv
```

Final verdict additions:

```text
IOS_GF_PRESET_PARITY: PASS / FAIL / NOT_EXECUTED
IOS_INFLIGHT_ACK_CLEANUP: PASS / FAIL / NOT_EXECUTED
IOS_DIVE_IMPORT_ACK_SYMMETRY: PASS / FAIL / NOT_EXECUTED
IOS_TOMBSTONE_SECURITY: PASS / FAIL / NOT_EXECUTED
IOS_SOFTWARE_READINESS_AFTER_REMEDIATION: <0-100>
```
...
# V1.7 IOS UNIFIED LOGBOOK AND GPS PRESENTATION SCOPE
...
LogbookView
IOSSnorkelingSessionsListView
IOSApneaSessionsListView
IOSUnifiedLogbookPresentationBuilderTests
IOSActivityLogbookVisibilitySettingsTests
IOSUnifiedLogbookNoContaminationTests
Docs/IOS_UNIFIED_ACTIVITY_LOGBOOK_VIEW.md
Docs/IOS_ACTIVITY_LOGBOOK_VISIBILITY_SETTINGS.md
Docs/IOS_UNIFIED_LOGBOOK_NO_CONTAMINATION_POLICY.md
```

Verify presentation-only aggregation, default OFF toggles, no merged store, no Watch sync mutation, fake/demo exclusion, and truthful GPS metadata display.

Additional outputs:

```text
Docs/MASTER_IOS_UNIFIED_LOGBOOK_AUDIT_CURRENT.md
Docs/MASTER_IOS_UNIFIED_LOGBOOK_PRESENTATION_ONLY_MATRIX_CURRENT.csv
Docs/MASTER_IOS_ACTIVITY_LOGBOOK_VISIBILITY_SETTINGS_MATRIX_CURRENT.csv
Docs/MASTER_IOS_GPS_LOGBOOK_PRESENTATION_MATRIX_CURRENT.csv
Docs/MASTER_IOS_UNIFIED_LOGBOOK_NO_CONTAMINATION_MATRIX_CURRENT.csv
```
