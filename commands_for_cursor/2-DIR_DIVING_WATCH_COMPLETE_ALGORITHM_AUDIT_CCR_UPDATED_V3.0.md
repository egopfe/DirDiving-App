# CURSOR / CODEX COMMAND — DIR DIVING WATCH COMPLETE ALGORITHM / SAFETY / RUNTIME AUDIT UPDATED WITH CCR & LATEST MAIN IMPLEMENTATIONS — V3.0
...
# V3.0 WATCH MULTI-ACTIVITY EXPANSION

This Watch audit now covers three production activities.

Add dedicated phases for:

- Watch startup activity selection;
- Diving Gauge;
- Diving Full Computer;
- Apnea lifecycle and recovery;
- Snorkeling surface GPS/dips/navigation;
- activity-owned Watch Settings;
- activity-specific session persistence;
- activity-discriminated sync;
- active-session mode lock;
- strict separation of Watch session models.

The previous rule forbidding Bühlmann on Watch is superseded where Full Computer is implemented in the current branch. Audit the actual implementation; do not assume it exists.

If Full Computer is present, verify live decompression authority, state machine, gas switching, recovery and fail-closed behavior. If absent or incomplete, report it without implementing it.
