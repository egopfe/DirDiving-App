...
**Canonical numbered filename:** `03-MASTER_UI_UX_FULL_DEEP_COMPREHENSIVE_AUDIT_COMMAND_V1.7.md`

---

# MASTER CURSOR / CODEX COMMAND — DIR DIVING UI/UX FULL DEEP COMPREHENSIVE AUDIT — V1.7
...
```text
Docs/MASTER_UI_UX_APNEA_FULL_DEEP_AUDIT_CURRENT.md
Docs/MASTER_UI_UX_APNEA_FEATURE_IMPLEMENTATION_MATRIX_CURRENT.csv
Docs/MASTER_UI_UX_APNEA_NAVIGATION_REACHABILITY_MATRIX_CURRENT.csv
Docs/MASTER_UI_UX_APNEA_STATE_COMPLETENESS_MATRIX_CURRENT.csv
Docs/MASTER_UI_UX_APNEA_ACCESSIBILITY_LOCALIZATION_MATRIX_CURRENT.csv
Docs/MASTER_UI_UX_ALGORITHMIC_TRUTHFULNESS_GATE_CURRENT.md
```
...
# V1.7 UI/UX SCOPE — SNORKELING P1/P2/P3 REMEDIATION AND NEW IOS FIXES

Verify user-visible latest changes:

```text
Snorkeling sync visibility on dashboard/list/detail/planner
stale route revision banner
Watch route pending/ready status and battery presentation
return-to-entry primary action visual/a11y contract
route summary on Watch ready panel
iOS re-send required banner
planned-vs-actual non-validation disclaimer
no production heatmap UI
CCR acknowledgement independent toggle and mode-aware copy
gas/cylinder section separated from checklist items
no fake/demo logs in real/unified logbook
```

Manual UI QA remains pending unless actual evidence exists.

Additional outputs:

```text
Docs/MASTER_UI_UX_V1_7_SNORKELING_REMEDIATION_AUDIT_CURRENT.md
Docs/MASTER_UI_UX_V1_7_CCR_ACK_COPY_AUDIT_CURRENT.md
Docs/MASTER_UI_UX_V1_7_EQUIPMENT_GAS_UI_AUDIT_CURRENT.md
Docs/MASTER_UI_UX_V1_7_DEMO_LOGBOOK_VISIBILITY_AUDIT_CURRENT.md
```
...
# 3B. POST-REMEDIATION UI/UX TRUTHFULNESS FOCUS

This audit must additionally verify whether remediation preserved or improved user-facing truthfulness for water auto-open, Apple Watch Auto-Launch setup, Action Button / Underwater Primary Action, Digital Crown clamp, shallow-depth developer toggles, Full Computer GF presets, sync/ACK states, and physical/external pending labels.

Mandatory negative checks:

```text
No screen claims DIR Diving always launches merely when the Apple Watch enters water unless physical watchOS Auto-Launch evidence exists.
No screen implies shallow-depth testing is production full-depth decompression guidance.
No screen hides that Full Computer water auto-open routes to predive confirmation.
No screen claims Action Button works underwater without user shortcut assignment and physical QA evidence.
No screen claims physical Water Lock, Crown, Action Button or shallow wet QA has passed unless evidence exists.
```

Additional required outputs:

```text
Docs/MASTER_UI_UX_POST_REMEDIATION_TRUTHFULNESS_AUDIT_CURRENT.md
Docs/MASTER_UI_UX_POST_REMEDIATION_COPY_CLAIMS_MATRIX_CURRENT.csv
Docs/MASTER_UI_UX_POST_REMEDIATION_ACCESSIBILITY_MATRIX_CURRENT.csv
Docs/MASTER_UI_UX_POST_REMEDIATION_WATER_AUTO_OPEN_MATRIX_CURRENT.csv
```

Final verdict additions:

```text
UI_UX_POST_REMEDIATION_TRUTHFULNESS: PASS / FAIL / NOT_EXECUTED
UI_UX_NO_UNSUPPORTED_WATER_AUTO_OPEN_CLAIMS: PASS / FAIL
UI_UX_NO_UNSUPPORTED_SHALLOW_DEPTH_CLAIMS: PASS / FAIL
UI_UX_SOFTWARE_READINESS_AFTER_REMEDIATION: <0-100>
UI_UX_PHYSICAL_QA_STATUS: PENDING_PHYSICAL / PASS / FAIL
```
...
# V1.7 UI/UX SCOPE — UNIFIED LOGBOOK, GPS METADATA AND PLANNER SEND ORDER

Verify:

```text
activity Settings expose independent Show all activities in logbook toggle
toggles default OFF and are presentation-only
unified list has activity badges and read-only state
detail navigation routes to activity-specific detail
no delete/edit/move/export in unified list unless implemented and audited
Diving GPS entry/exit displayed truthfully
Apnea GPS surface start/end displayed truthfully
Snorkeling GPS track/quality displayed truthfully
no UI claims underwater GPS or decompression authority
planner incomplete-state / route-safety messaging appears before Watch send actions
manual UI QA remains pending unless executed
```

Additional outputs:

```text
Docs/MASTER_UI_UX_UNIFIED_LOGBOOK_AUDIT_CURRENT.md
Docs/MASTER_UI_UX_UNIFIED_LOGBOOK_REACHABILITY_MATRIX_CURRENT.csv
Docs/MASTER_UI_UX_GPS_LOGBOOK_TRUTHFULNESS_MATRIX_CURRENT.csv
Docs/MASTER_UI_UX_PLANNER_SEND_ORDER_MATRIX_CURRENT.csv
Docs/MASTER_UI_UX_MANUAL_UI_QA_PENDING_MATRIX_CURRENT.csv
```
