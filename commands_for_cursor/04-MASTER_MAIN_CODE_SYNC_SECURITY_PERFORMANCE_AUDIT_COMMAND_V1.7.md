...
**Canonical numbered filename:** `04-MASTER_MAIN_CODE_SYNC_SECURITY_PERFORMANCE_AUDIT_COMMAND_V1.7.md`

---

# MASTER CURSOR / CODEX COMMAND — DIR DIVING MAIN CODE / SYNC / SECURITY / PERFORMANCE AUDIT — V1.7
...
# V1.7 MAIN CODE / SYNC / SECURITY / PERFORMANCE SCOPE — SNORKELING REMEDIATION

Verify:

```text
Snorkeling pending route queue persistence has safe serialization/corruption handling.
Pending route queue namespace cannot collide with Apnea/Diving.
ACK clear logic prevents stuck queue and duplicate unsafe send.
WatchConnectivity E2E remains PENDING_PAIRED_DEVICE_QA unless executed.
No production heatmap.
No Always Location.
No demo/fake data in real stores.
No performance regression from planned-vs-actual analytics or micro-map.
```

Additional outputs:

```text
Docs/MASTER_MAIN_V1_7_SNORKELING_SYNC_PERSISTENCE_AUDIT_CURRENT.md
Docs/MASTER_MAIN_V1_7_SNORKELING_SECURITY_PRIVACY_MATRIX_CURRENT.csv
Docs/MASTER_MAIN_V1_7_SNORKELING_PERFORMANCE_RISK_MATRIX_CURRENT.csv
Docs/MASTER_MAIN_V1_7_DEMO_DATA_ISOLATION_MATRIX_CURRENT.csv
```
...
commands_for_cursor/07-MASTER_POST_REMEDIATION_CODE_READINESS_VERIFICATION_AUDIT_COMMAND_V1.7.md
```
...
# V1.7 MAIN CODE / SYNC / SECURITY SCOPE — GPS, UNIFIED LOGBOOK AND TEST EVIDENCE

Verify:

```text
When In Use-only location policy
no Always Location unless separately audited
no fake/demo contamination
no cross-activity writes
no sync namespace contamination
no Watch runtime mutation from unified iOS presentation
no performance regression from aggregating large logbooks
1655/1655 iOS and 1152/1152 Watch PASS evidence is consumed only as software/test evidence
physical/manual gates remain pending
```

Additional outputs:

```text
Docs/MASTER_MAIN_GPS_LOGBOOK_SYNC_SECURITY_AUDIT_CURRENT.md
Docs/MASTER_MAIN_UNIFIED_LOGBOOK_NO_CONTAMINATION_AUDIT_CURRENT.csv
Docs/MASTER_MAIN_LOCATION_PRIVACY_MATRIX_CURRENT.csv
Docs/MASTER_MAIN_V1_6_TEST_EVIDENCE_MATRIX_CURRENT.csv
Docs/MASTER_MAIN_UNIFIED_LOGBOOK_PERFORMANCE_RISK_MATRIX_CURRENT.csv
```
