...
**Canonical numbered filename:** `01-MASTER_WATCH_FULL_COMPUTER_FORENSIC_AUDIT_COMMAND_V1.7.md`

---

# MASTER CURSOR / CODEX COMMAND — DIR DIVING APPLE WATCH FULL COMPUTER FULL DEEP FORENSIC AUDIT — V1.7
...
# V1.7 WATCH-SPECIFIC SNORKELING AND FULL COMPUTER BOUNDARY AUDIT

Audit that Snorkeling Watch P1/P2/P3 remediation did not affect Diving Full Computer.
...
Verify that Snorkeling UI/runtime code cannot mutate Full Computer tissues, GF, gas state, NDL, TTS, ceiling or decompression schedule.

Additional output:

```text
Docs/MASTER_WATCH_V1_7_SNORKELING_FULL_COMPUTER_BOUNDARY_AUDIT_CURRENT.md
```
...
# 4B. POST-REMEDIATION WATCH FULL COMPUTER FOCUS

This audit must additionally verify whether remediation has closed these consolidated software findings where they affect Watch Full Computer:

```text
CONS-002 — iOS GF preset mismatch affecting imported Watch Full Computer plans
CONS-006 — shallow Full Computer internal testing toggle release exposure
CONS-007 — depth tier authority must not trust Info.plist alone
CONS-008 — TTS/schedule oracle independence
CONS-016 — Watch Full Computer secondary mathematical/test drift if present in the consolidated register
```

Additional required outputs:

```text
Docs/MASTER_WATCH_FULL_COMPUTER_POST_REMEDIATION_VERIFICATION_CURRENT.md
Docs/MASTER_WATCH_FULL_COMPUTER_GF_IMPORT_POST_REMEDIATION_MATRIX_CURRENT.csv
Docs/MASTER_WATCH_FULL_COMPUTER_DEPTH_CAPABILITY_AUTHORITY_MATRIX_CURRENT.csv
Docs/MASTER_WATCH_FULL_COMPUTER_ORACLE_INDEPENDENCE_MATRIX_CURRENT.csv
```

Final verdict additions:

```text
WATCH_FC_GF_IMPORT_PARITY: PASS / FAIL / NOT_EXECUTED
WATCH_FC_DEPTH_CAPABILITY_AUTHORITY: PASS / FAIL / NOT_EXECUTED
WATCH_FC_INDEPENDENT_ORACLE: PASS / PARTIAL_PENDING_EXTERNAL / FAIL / NOT_EXECUTED
WATCH_FC_SOFTWARE_READINESS_AFTER_REMEDIATION: <0-100>
WATCH_FC_PHYSICAL_QA_STATUS: PENDING_PHYSICAL / PASS / FAIL
```
...
# V1.7 WATCH-SPECIFIC GPS / LOGBOOK SCOPE

Inspect Watch-side GPS capture and activity-specific logbook persistence.

```text
WatchSurfaceLocationService
GPSManager
DiveManager entryGPS / exitGPS
ApneaWatchRuntimeStore.attachSurfaceLocation
ApneaSessionEngine.surfaceGPSPoints
ApneaLogbookStore
Snorkeling GPS runtime evaluator
WatchDiveSyncCodec
ApneaSessionSyncCodec
Snorkeling sync codecs
Docs/QA_EVIDENCE/WATCH_GPS_*
```

Verify GPS never affects Full Computer tissues, gas state, GF, NDL, TTS, ceiling or schedule.

Additional outputs:

```text
Docs/MASTER_WATCH_GPS_ACTIVITY_LOGBOOK_AUDIT_CURRENT.md
Docs/MASTER_WATCH_GPS_ACTIVITY_LOGBOOK_MATRIX_CURRENT.csv
Docs/MASTER_WATCH_FULL_COMPUTER_GPS_NON_REGRESSION_GATE_CURRENT.md
```
...
In addition to the V2.2 underwater hardware scope, audit the following new UI/UX surfaces.
...
```text
Docs/MASTER_WATCH_FULL_COMPUTER_ALGORITHMIC_SAFETY_GATE_CURRENT.md
Docs/MASTER_WATCH_FULL_COMPUTER_APNEA_BOUNDARY_AUDIT_CURRENT.md
Docs/MASTER_WATCH_FULL_COMPUTER_APNEA_BOUNDARY_MATRIX_CURRENT.csv
Docs/MASTER_WATCH_APNEA_WATER_AUTO_OPEN_BOUNDARY_MATRIX_CURRENT.csv
```
