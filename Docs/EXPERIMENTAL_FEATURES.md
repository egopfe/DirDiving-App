...
- `origin/main` @ `9ff45ac`
- `origin/codex/experimental-features` (this branch)

`codex/experimental-features` is the canonical combined experimental branch. Apnea and Snorkeling remain experimental-only on this branch. Buddy Assist, BLE relay, peer pairing, and messaging remain lab-only and must not be promoted with an Apnea/Snorkeling-only merge.
