# Physical QA — SNORKELING_PAIR_UNPAIR

| Field | Value |
|-------|-------|
| **QA ID** | SNK-QA-018 |
| **Command category** | SNORKELING_PAIRED_DEVICE_MATRIX |
| **Status** | **PENDING** |
| **Branch** | (record at execution) |
| **Commit** | (record at execution) |
| **Purpose** | Pair/unpair and multi-device matrix |
| **Required device** | Multiple paired iPhone/Watch combinations |
| **Tester** | |
| **Reviewer** | |
| **Execution date** | |
| **iPhone model** | |
...
Test matrix documented before execution

## Test steps

1. Install the build at the recorded commit SHA.
2. Execute the scenario for **SNORKELING_PAIR_UNPAIR** per `PROCEDURE.md` when present.
