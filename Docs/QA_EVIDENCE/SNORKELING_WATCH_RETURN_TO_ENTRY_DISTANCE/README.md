# Physical QA — SNORKELING_WATCH_RETURN_TO_ENTRY_DISTANCE

| Field | Value |
|-------|-------|
| **QA ID** | SNK-QA-031 |
| **Priority** | P1 |
| **Command category** | SNORKELING_WATCH_P1_P2_P3 |
| **Status** | **PENDING** |
| **Branch** | (record at execution) |
| **Commit** | (record at execution) |
| **Purpose** | Watch return screen shows entry distance and bearing outdoors |
| **Required device** | Physical Apple Watch + iPhone |
| **Tester** | |
| **Reviewer** | |
| **Execution date** | |
| **iPhone model** | |
...
Active session with entry captured; outdoor GPS

## Test steps

1. Install the build at the recorded commit SHA.
2. Execute the scenario for **SNORKELING_WATCH_RETURN_TO_ENTRY_DISTANCE** per `PROCEDURE.md` when present.
