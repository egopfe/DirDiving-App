# Physical QA — SNORKELING_WATCH_RETURN_ALERT
...
| **Purpose** | 50% planned time/distance return alert haptic fires once |
...
Planned route with return alert policy enabled

## Test steps

1. Install the build at the recorded commit SHA.
2. Execute the scenario for **SNORKELING_WATCH_RETURN_ALERT** per `PROCEDURE.md` when present.
