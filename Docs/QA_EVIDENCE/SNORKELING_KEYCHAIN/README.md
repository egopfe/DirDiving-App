# Physical QA — SNORKELING_KEYCHAIN

| Field | Value |
|-------|-------|
| **QA ID** | SNK-QA-021 |
| **Command category** | SNORKELING_IOS_WATCH_SYNC |
| **Status** | **PENDING** |
| **Branch** | (record at execution) |
| **Commit** | (record at execution) |
| **Purpose** | Sync crypto keychain trust and rotation |
...
Fresh install or key rotation scenario

## Test steps

1. Install the build at the recorded commit SHA.
2. Execute the scenario for **SNORKELING_KEYCHAIN** per `PROCEDURE.md` when present.
