# Physical QA — SNORKELING_OFFLINE_ONLINE

| Field | Value |
|-------|-------|
| **QA ID** | SNK-QA-013 |
| **Command category** | SNORKELING_OFFLINE |
| **Status** | **PENDING** |
| **Branch** | (record at execution) |
| **Commit** | (record at execution) |
| **Purpose** | Offline route/session behavior and reconnect sync |
| **Required device** | Paired iPhone + Watch |
| **Tester** | |
| **Reviewer** | |
| **Execution date** | |
| **iPhone model** | |
...
Airplane mode or unreachable peer

## Test steps

1. Install the build at the recorded commit SHA.
2. Execute the scenario for **SNORKELING_OFFLINE_ONLINE** per `PROCEDURE.md` when present.
