# Physical QA — SNORKELING_WATER_LOCK

| Field | Value |
|-------|-------|
| **QA ID** | SNK-QA-004 |
| **Command category** | SNORKELING_WATER_LOCK |
| **Status** | **PENDING** |
| **Branch** | (record at execution) |
| **Commit** | (record at execution) |
| **Purpose** | Water Lock during Snorkeling session |
...
Active or pre-session Snorkeling UI

## Test steps

1. Install the build at the recorded commit SHA.
2. Execute the scenario for **SNORKELING_WATER_LOCK** per `PROCEDURE.md` when present.
