...
**Command:** `01-MASTER_WATCH_FULL_COMPUTER_FORENSIC_AUDIT_COMMAND_V1.5.md`  
...
|---|---:|---:|---|---|---|
| Tissue pressure (N2/He bar) | 0.0001 | < 1e-5 in ML profiles | Either | YES | Audit15MultilevelOracleProfilesTests @2c30412 |
...
| TTS (min) | 3.0 | ≤ 3 in oracle sweep | Over-estimate safer | YES | Audit15TTSScheduleOracleSweepTests |
| Planner vs runtime TTS | 4.0 | Within tolerance @2c30412 | Over-estimate safer | YES | testPlannerRuntimeTTSWithinTolerance PASS 0.474s |
...
|---|---:|---:|---|---|---|
| Float vs Double | Production uses Double | N/A | N/A | YES | BuhlmannConstants Double arrays |
| exp underflow near saturation | Clamped finite guards | Stable | Stable | YES | schreiner guard k>0 finite |
| 1-min TTS forward quanta | +0–1 min TTS | +≤1 min | Conservative | YES | CONS-016 documented |
| 30s Schreiner sub-steps | Sub-split of analytic segment | Bounded | Bounded | YES | maxSubStepSeconds=30 |
| Missed tick actual-dt integration | Full elapsed integrated | No under-exposure | YES | FullComputerTimingFaultTests |
| Display rounding vs canonical | 0.1 m ceiling display | Exact for violations | Conservative | YES | ceilingViolation uses exact |
| Cache stale on budget exceed | Prior presentation | Conservative | YES | FullComputerDecoSolverCacheIsolationTests |
| Altitude surface pressure ISA | ISA barometric | Standard model | YES | AmbientPressureModel + oracle |
...
| dt > maxMissedTickSeconds (120s) | degraded; tissues integrated | FullComputerTimingFaultTests |
| Out-of-order timestamp | degraded non_monotonic_timestamp | ingestSample |
| Invalid GF | canStart fails | FullComputerRuntimePlan.validate |
...
- **Long-run drift (>4h deco):** Bounded by maxScheduleMinutes=720; not oracle-swept beyond test profiles.
- **Altitude trimix ML replay:** ALT-009 NOT_EXECUTED at elevation (WFC-P2-003).
