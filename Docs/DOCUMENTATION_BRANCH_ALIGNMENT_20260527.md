...
|----|--------------|-----------|------|----------------|
| #8 `Update experimental Apnea workflow` | `codex/experimental-features` -> `main` | `UNKNOWN` / `CONFLICTING` observed | high runtime risk | do not auto-merge |
...
Both PRs are experimental and require manual review, macOS/Xcode validation and explicit safety signoff before any merge. PR #8 must be treated as unsafe while GitHub alternates between `UNKNOWN` and `CONFLICTING`.
