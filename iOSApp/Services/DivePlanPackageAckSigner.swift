...
enum DivePlanPackageAckSigner {
    static func makeSignature(planID: UUID, revision: Int, checksum: String, issuedAt: Date) -> String {
        guard WatchSyncAuth.hasPeerSecret(),
              let key = try? WatchSyncAuth.deriveSyncKey(peerBundleID: "com.egopfe.dirdiving.ios.watch") else {
            return ""
        }
        let canonical = DivePlanPackageCodec.ackCanonical(
            planID: planID,
            revision: revision,
            checksum: checksum,
            issuedAt: issuedAt
    ...
    static func verify(
        ...
        let expected = makeSignature(planID: planID, revision: revision, checksum: checksum, issuedAt: issuedAt)
