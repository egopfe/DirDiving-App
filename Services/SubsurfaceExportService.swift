...
enum SubsurfaceExportService {
    static func makeCSV(
        ...
        rows.append(contentsOf: metadataLines(for: session, privacyOptions: privacyOptions))
        let manualMeta = [
    ...
    static func writeCSV(
        ...
        guard let csv = makeCSV(for: session, privacyOptions: privacyOptions) else { return nil }
