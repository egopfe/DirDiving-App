...
final class SnorkelingRoutePlannerSectionOrderTests: XCTestCase {
    func testRoutePlannerSectionOrderIsMapThenPointsThenProfiles() throws {
        ...
        let safetyIndex = try XCTUnwrap(source.range(of: "routeSafetySection", range: profilesIndex..<source.endIndex)?.lowerBound)
        let transferIndex = try XCTUnwrap(source.range(of: "transferSection", range: safetyIndex..<source.endIndex)?.lowerBound)
        let sendIndex = try XCTUnwrap(source.range(of: "snorkeling.ios.planner.send_watch", range: transferIndex..<source.endIndex)?.lowerBound)
        XCTAssertLessThan(mapIndex, pointsIndex)
        XCTAssertLessThan(pointsIndex, profilesIndex)
        XCTAssertLessThan(profilesIndex, safetyIndex)
        XCTAssertLessThan(safetyIndex, transferIndex)
